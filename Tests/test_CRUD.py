...
def Test_Adauga_Rezervare():
    lista = Adauga_Rezervare("1", "Suciu", "business", 100, "Da", [])
    assert len(lista) == 1
    assert get_ID(get_by_ID("1", lista)) == "1"
    assert get_nume(get_by_ID("1", lista)) == "Suciu"
...
def Test_Sterge_Rezervare():
    lista = Adauga_Rezervare("1", "Suciu", "business", 100, "Da", [])
    lista = Adauga_Rezervare("2", "Sergiu", "economy", 20, "Nu", lista)
...
def Test_Modifica_Rezervare():
    lista = Adauga_Rezervare("1", "Suciu", "business", 100, "Da", [])
    lista = Adauga_Rezervare("2", "Sergiu", "economy", 20, "Nu", lista)
    lista = Modifica_Rezervare("2", "Eduard", "economy plus", 20, "Nu", lista)
    assert get_ID(get_by_ID("2", lista)) == "2"
    assert get_nume(get_by_ID("2", lista)) == "Eduard"
